...
def ensure(path, cols):
    if not os.path.exists(path):
        pd.DataFrame(columns=cols).to_csv(path,index=False); print("[OK] created",path); return
    try:
        df=pd.read_csv(path); 
        if len(df.columns)==0: raise ValueError("no header")
    except Exception:
        pd.DataFrame(columns=cols).to_csv(path,index=False); print("[OK] fixed",path)
