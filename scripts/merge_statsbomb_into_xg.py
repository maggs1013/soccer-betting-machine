...
# Uses StatsBomb team totals (xg_sb/xga_sb) to fill missing xg_hybrid/xga_hybrid/xgd90_hybrid.
# We DO NOT override existing hybrid values.
...
def main():
    if not os.path.exists(HYB):
        print("[INFO] hybrid missing; skip StatsBomb merge.")
        return
    hyb = pd.read_csv(HYB)
    if hyb.empty or "team" not in hyb.columns:
        print("[INFO] hybrid empty or lacks 'team'; skip.")
        return

    if not os.path.exists(SB):
        print("[INFO] xg_statsbomb.csv missing; skip.")
        return
    sb = pd.read_csv(SB)
    if sb.empty or "team" not in sb.columns:
        print("[INFO] StatsBomb file empty; skip.")
        return

    # Convert to per-match proxy. StatsBomb open data doesn't always include matches count, so use nominal 34.
    sb["xg_sb_p90"]  = pd.to_numeric(sb["xg_sb"], errors="coerce") / 34.0
    sb["xga_sb_p90"] = pd.to_numeric(sb["xga_sb"], errors="coerce") / 34.0
    sb_small = sb.groupby("team", as_index=False)[["xg_sb_p90","xga_sb_p90"]].mean()  # average across seasons in report
