...
def table_by(df, by_cols, title, sort_cols=None):
    if df.empty:
        return [f"\n### {title}\n- (no data)\n"]
    cols = ["model","n","hit_rate","brier","ece_weighted","roi","pnl_units"]
    show_cols = [c for c in by_cols if c in df.columns] + [c for c in cols if c in df.columns]
    ...
    with open(OUT, "a", encoding="utf-8") as f:
        ...
        for lg in leagues:
            ...
            for bet in ["1X2","OU","BTTS"]:
                ...
                lines = table_by(seg, by_cols=["prob_bin"], title=f"{bet} — by prob_bin",
                                 sort_cols=["prob_bin","model"])
                f.write("\n".join(lines))
                # Table by odds bin (only if 1X2 & any odds_bin not NaN)
                if bet=="1X2" and seg["odds_bin"].notna().any():
                    lines = table_by(seg[seg["odds_bin"].notna()], by_cols=["odds_bin"], 
                                     title=f"{bet} — by odds_bin", sort_cols=["odds_bin","model"])
                    f.write("\n".join(lines))
