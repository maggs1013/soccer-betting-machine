...
- Fetch each slice from soccerdata, clean keys, and write to data/fbref_slice_<canonical>.csv
- Writes manifest: data/fbref_slices_manifest.json
- No large in-memory merge here (merging is deferred to enrichment to avoid runner OOM)
"""
...
def _drop_nonkey_allna(df):
    drop=[c for c in df.columns if c not in KEYS and df[c].isna().all()]
    return df.drop(columns=drop) if drop else df

def _clean_to_disk(df: pd.DataFrame, canonical: str) -> bool:
    ...
    print(f"[FBref] saved {canonical} → {out} shape={df.shape}")
    return True
...
def _try_stat_type(fb, canonical: str) -> bool:
    for candidate in CANONICAL_SLICES[canonical]:
        ...
        if raw is None or raw.empty:
            ...
            continue
        if _clean_to_disk(raw, canonical):
